...
# AdamAnSubtracM's Eslint & Prettier Setup

## Note: I straight up stole most of this from Wes Bos. All credit should go to him
...
```bash
npx install-peerdeps --global eslint-config-adamansubtractm
```
...
1. Run `npx install-peerdeps --dev eslint-config-adamansubtractm`
1. Crack open your `package.json` and replace `"extends": "react-app"` with `"extends": "adamansubtractm"`
...
Needs some instructions [from here](https://github.com/wesbos/eslint-config-adamansubtractm/wesbos/33)
...
```bash
npm remove --global eslint-config-adamansubtractm babel-eslint eslint eslint-config-prettier eslint-config-airbnb eslint-plugin-html eslint-plugin-prettier eslint-plugin-import eslint-plugin-jsx-a11y eslint-plugin-react prettier eslint-plugin-react-hooks
```
